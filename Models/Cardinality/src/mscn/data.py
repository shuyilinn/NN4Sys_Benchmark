...
def encode_query(queries, num_materialized_samples, save_path, test_data_name):
    ...
    dataset = make_dataset(samples_test, predicates_test, joins_test, labels_test, max_num_samples, max_num_joins, max_num_predicates, sample_feats, predicate_feats, join_feats)

    return dataset, min_val, max_val, max_num_samples, max_num_predicates, max_num_joins
...
def zero_padding(samples, predicates, joins, labels, max_num_samples, max_num_joins, max_num_predicates, sample_feats, predicate_feats, join_feats):
    ...
    for sample in samples:
        ...
        num_pad = max_num_samples + 1 - sample_tensor.shape[0]
...
def make_dataset(samples, predicates, joins, labels, max_num_samples, max_num_joins, max_num_predicates, sample_feats, predicate_feats, join_feats):
    ...
    new_joins = []
    new_labels = []
    for i,j,k,v in zip(samples, predicates, joins, labels):
    ...
    return dataset.TensorDataset(*zero_padding(new_samples, new_predicates, new_joins, new_labels, max_num_samples, max_num_joins, max_num_predicates, sample_feats, predicate_feats, join_feats))
...
def get_train_datasets(num_queries, num_materialized_samples):
    ...
    predicate_feats = len(column2vec) + len(op2vec) + 1
    join_feats = len(join2vec)
    train_dataset = make_dataset(*train_data, labels=labels_train, max_num_samples=max_num_samples, max_num_joins=max_num_joins,
    ...
    test_dataset = make_dataset(*test_data, labels=labels_test, max_num_samples=max_num_samples, max_num_joins=max_num_joins,
                                max_num_predicates=max_num_predicates, sample_feats=sample_feats, predicate_feats=predicate_feats, join_feats=join_feats)
