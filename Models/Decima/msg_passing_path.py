...
def get_running_dag_mat(job_dags):
    ...
    running_dag_indices = np.mat(
        [running_dag_row_idx, running_dag_col_idx])
    running_dag_mat = torch.sparse_coo_tensor(
        torch.tensor(running_dag_indices), running_dag_data, running_dag_shape)
...
def get_unfinished_nodes_summ_mat(job_dags):
    ...
    summ_indices = np.mat([summ_row_idx, summ_col_idx])
    summerize_mat = torch.sparse_coo_tensor(
