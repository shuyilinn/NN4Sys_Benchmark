...
def main():
	...
	for log_file in log_files:
		...
		for scheme in SCHEMES:
			...
			if True:
				...
				# print(raw_reward_all[scheme][log_file[len('log_' + str(scheme) + '_'):]], "======")
				#break
	...
	for l in time_all[SCHEMES[0]]:
		schemes_check = True
		for scheme in SCHEMES:
			if l not in list(time_all[scheme].keys()) or len(time_all[scheme][l]) < VIDEO_LEN:
				schemes_check = False
				break
		if schemes_check:
	...
	for scheme in SCHEMES:
		values, base = np.histogram(reward_all[scheme], bins=NUM_BINS, density=False)
		cumulative = np.cumsum(values).astype(np.float64)


		length = cumulative.shape[0]
		for i in range(length):

			cumulative[i] = float(cumulative[i])/cumulative[length-1]
