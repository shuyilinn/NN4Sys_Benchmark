"""
This script runs ABCrown verification for Pensieve models using ONNX models and VNNLIB specifications.
It generates YAML configurations for each model and saves the results in the specified directory.
"""

import os
import sys
import argparse

# Set MKL threading layer to GNU
os.environ['MKL_THREADING_LAYER'] = 'GNU'

# Model configurations
MODEL_TYPES = ['simple', 'simple', 'parallel']  # Types of models
MODEL_SIZES = ['small', 'mid', 'big']  # Model sizes
P_RANGE = [0.05, 0.1, 0.5, 0.7, 1]  # Parameter ranges
SPEC_TYPES = [1, 2, 3]  # Specification types
DIMENSION_NUMBERS = [1, 2, 3]  # Dimension numbers

# Paths
vnn_dir_path = '../Benchmarks/vnnlib'  # Path for VNNLIB files
onnx_dir_path = '../Benchmarks/onnx'  # Path for ONNX models
yaml_path = './pensieve_yaml'  # Path for YAML configuration files
running_result_path = './pensieve_abcrown_running_result'  # Path for saving results

# Ensure necessary directories exist
if not os.path.exists(running_result_path):
    os.makedirs(running_result_path)
if not os.path.exists(yaml_path):
    os.makedirs(yaml_path)

def create_yaml(yaml, vnn_path, onnx_path, inputshape=6):
    """
    Creates a YAML configuration file for a specific Pensieve model.

    Args:
        yaml (str): Path to the YAML file to create.
        vnn_path (str): Path to the VNNLIB file.
        onnx_path (str): Path to the ONNX model file.
        inputshape (int): Input shape for the model.
    """
    with open(yaml, mode='w') as f:
        f.write("general:\n  enable_incomplete_verification: False\n  conv_mode: matrix\n")
        f.write(f'model:\n  onnx_path: {onnx_path}\n')
        f.write('  onnx_quirks: "{\'Reshape\': {\'fix_batch_size\': True}}"\n')
        f.write(f'  input_shape: [-1, {inputshape}, 8]\n')
        f.write(f'specification:\n  vnnlib_path: {vnn_path}\n')
        f.write(
            "solver:\n  batch_size: 1\nbab:\n  branching:\n    method: sb\n    sb_coeff_thresh: 0.1\n    input_split:\n      enable: True"
        )

def main(abcrown_path, size=10):
    """
    Main function to run ABCrown verification for Pensieve models.

    Args:
        abcrown_path (str): Path to the ABCrown verifier executable/script.
        size (int): Number of verification instances to run for each model (default: 10).
    """
    for i in range(len(SPEC_TYPES)):
        MODEL_TYPE = MODEL_TYPES[i]

        for range_ptr in range(len(P_RANGE)):
            for d_ptr in range(len(DIMENSION_NUMBERS)):
                dimension_number = DIMENSION_NUMBERS[d_ptr]
                for MODEL in MODEL_SIZES:
                    for instance in range(size):
                        if dimension_number != 2 or range_ptr != 0:
                            continue

                        # Construct paths for VNNLIB, ONNX model, and YAML configuration
                        vnn_path = f'{vnn_dir_path}/pensieve_{SPEC_TYPES[i]}_{dimension_number}_{range_ptr}_{instance}.vnnlib'
                        onnx_path = onnx_dir_path + f'/pensieve_{MODEL}_{MODEL_TYPE}.onnx'
                        yaml = yaml_path + f'/pensieve_{MODEL_TYPE}_{MODEL}_{SPEC_TYPES[i]}_{dimension_number}_{range_ptr}_{instance}.yaml'

                        # Create YAML configuration
                        if MODEL_TYPE == 'simple':
                            create_yaml(yaml, vnn_path, onnx_path, 6)
                        if MODEL_TYPE == 'parallel':
                            create_yaml(yaml, vnn_path, onnx_path, 12)

                        # Run ABCrown with the generated YAML configuration and save the output
                        result_path = f'{running_result_path}/pensieve_{MODEL}_{SPEC_TYPES[i]}_{dimension_number}_{range_ptr}_{instance}.txt'
                        command = f"python {abcrown_path} --config {yaml} | tee {result_path}"

                        # Print and execute the command
                        print("--------------------------------------------------------------------------->")
                        print(command)
                        print("<---------------------------------------------------------------------------")
                        os.system(command)

if __name__ == "__main__":
    # Argument parser setup
    parser = argparse.ArgumentParser(description="Run ABCrown verification for Pensieve models.")
    
    # Add argument for the path to the ABCrown verifier
    parser.add_argument("abcrown_path", help="Path to the ABCrown verifier.")
    
    # Add optional argument for specifying the number of verification instances (default: 10)
    parser.add_argument("--size", type=int, default=10, help="Number of verification instances to run (default: 10).")
    
    # Parse the command-line arguments
    args = parser.parse_args()

    # Call the main function with the parsed arguments
    main(args.abcrown_path, args.size)
